...
import qt.helpers;
import qt.webengine.contextmenurequest;
...
public:
    ...
    final void setContent(ref const(QByteArray) data, ref const(QString) mimeType = globalInitVar!QString,
                        ref const(QUrl) baseUrl = globalInitVar!QUrl);
...
/+ #endif +/
    final QWebEngineContextMenuRequest lastContextMenuRequest() const;

    final void printToPdf(ref const(QString) filePath,
                        ref const(QPageLayout) layout /+ = QPageLayout(QPageSize(QPageSize::A4),
    ...
    final void print(QPrinter* printer);
...
/+ Q_SIGNALS +/public:
    ...
    @QSignal final void printFinished(bool success);
