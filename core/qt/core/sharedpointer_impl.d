 ...
 */
module qt.core.sharedpointer_impl;
extern(C++):
...
QSharedPointer!(X) qSharedPointerCast(X, T)(ref const(QSharedPointer!(T)) ptr);
QSharedPointer!(X) qSharedPointerDynamicCast(X, T)(ref const(QSharedPointer!(T)) ptr);
QSharedPointer!(X) qSharedPointerConstCast(X, T)(ref const(QSharedPointer!(T)) ptr);
...
extern(C++, "QtSharedPointer") {
    /+ template <class T> class ExternalRefCount; +/

    QSharedPointer!(X) copyAndSetPointer(X, Y)(X*  ptr, ref const(QSharedPointer!(Y)) src);
...
/*    pragma(inline, true) void executeDeleter(T, Klass, RetVal)(T* t, ExternCPPFunc!(RetVal function())/+ Klass:: * +/ memberDeleter)
    { if (t) (t->*memberDeleter)(); }
    pragma(inline, true) void executeDeleter(T, Deleter)(T* t, Deleter d)
    { d(t); }*/
    ...
    {
        alias DestroyerFn = ExternCPPFunc!(void function(ExternalRefCountData* ));
        QBasicAtomicInt weakref;
        QBasicAtomicInt strongref;
        DestroyerFn destroyer;

        pragma(inline, true) this(DestroyerFn d)
        {
            this.destroyer = d;
        ...
        pragma(inline, true) this(/+ Qt:: +/qt.core.namespace.Initialization) { }
        ~this() { (mixin(Q_ASSERT(q{!ExternalRefCountData.weakref.loadRelaxed()}))); (mixin(Q_ASSERT(q{ExternalRefCountData.strongref.loadRelaxed() <= 0}))); }
...
/+ #endif +/
        /*pragma(inline, true) void checkQObjectShared(...) { }
        pragma(inline, true) void setQObjectShared(...) { }*/
    ...
    struct CustomDeleter(T, Deleter)
    {
        Deleter deleter;
        T* ptr;
...
public:
    ...
    /+ /+ explicit +/pragma(inline, true) this(X,)(X* ptr)
    // noexcept
{
    this.value = ptr;
    internalConstruct(ptr, /+ QtSharedPointer:: +/NormalDeleter());
    ...
    pragma(inline, true) this(X,Deleter,)(X* ptr, Deleter deleter)
    // throws
{
    this.value = ptr;
    ...
    /*this(Deleter)(/+ std:: +/nullptr_t, Deleter deleter)
    {
        this.value = null;
        internalConstruct(static_cast!(T*)(null), deleter);
    }*/
    ...
    pragma(inline, true) void reset(T* t)
    { auto copy = QSharedPointer(t); swap(copy); }*/
    /+ /+ template <typename Deleter> +/
    pragma(inline, true) void reset(Deleter)(T* t, Deleter deleter)
    { auto copy = QSharedPointer(t, deleter); swap(copy); }+/

    /+/+ template <class X> +/
    QSharedPointer!(X) staticCast(X)() const
    {
        return qSharedPointerCast!(X, T)(this);
    }

    /+ template <class X> +/
    QSharedPointer!(X) dynamicCast(X)() const
    {
        return qSharedPointerDynamicCast!(X, T)(this);
    }

    /+ template <class X> +/
    QSharedPointer!(X) constCast(X)() const
    {
...
/+ #ifndef QT_NO_QOBJECT +/
    /+ template <class X> +/
    /*QSharedPointer!(X) objectCast(X)() const
    {
        return qSharedPointerObjectCast!(X, T)(this);
    }*/
/+ #endif +/

    //pragma(inline, true) void clear() { QSharedPointer copy; swap(copy); }
...
private:
    ...
    /+ template <class X> +/
    /+pragma(inline, true) void enableSharedFromThis(X)(const(QEnableSharedFromThis!(X))* ptr)
    {
        ptr.initializeFromSharedPointer(constCast!(/+ std:: +/remove_cv!(T).type)());
    }+/

    //pragma(inline, true) void enableSharedFromThis(...) {}

    /+ template <typename X, typename Deleter> +/
    /*pragma(inline, true) void internalConstruct(X,Deleter)(X* ptr, Deleter deleter)
    {
        alias Private = /+ QtSharedPointer:: +/ExternalRefCountWithCustomDeleter!(X, Deleter);
        static if (defined!"QT_SHAREDPOINTER_TRACK_POINTERS")
        {
            Private.DestroyerFn actualDeleter = &Private.safetyCheckDeleter;
        }
        else
        {
            Private.DestroyerFn actualDeleter = &Private.deleter;
        }
...
struct EnableInternalDataWrap(T) {
    EnableInternalData base0;
    alias base0 this;
}
}
...
// operator-
//
/+pragma(inline, true) QSharedPointer!(T).difference_type operator -(T, X)(ref const(QSharedPointer!(T)) ptr1, ref const(QSharedPointer!(X)) ptr2)
{
    return ptr1.data() - ptr2.data();
}+/
/+pragma(inline, true) QSharedPointer!(T).difference_type operator -(T, X)(ref const(QSharedPointer!(T)) ptr1, X* ptr2)
{
    return ptr1.data() - ptr2;
}+/
/+pragma(inline, true) QSharedPointer!(X).difference_type operator -(T, X)(T* ptr1, ref const(QSharedPointer!(X)) ptr2)
{
...
// operator<
//
/+pragma(inline, true) bool operator <(T, X)(ref const(QSharedPointer!(T)) ptr1, ref const(QSharedPointer!(X)) ptr2)
{
    alias CT = UnknownType!q{/+ std:: +/common_type!(T*, X*).type};
    return /+ std:: +/less!(CT)()(ptr1.data(), ptr2.data());
}+/
/+pragma(inline, true) bool operator <(T, X)(ref const(QSharedPointer!(T)) ptr1, X* ptr2)
{
    alias CT = UnknownType!q{/+ std:: +/common_type!(T*, X*).type};
    return /+ std:: +/less!(CT)()(ptr1.data(), ptr2);
}+/
/+pragma(inline, true) bool operator <(T, X)(T* ptr1, ref const(QSharedPointer!(X)) ptr2)
{
    alias CT = UnknownType!q{/+ std:: +/common_type!(T*, X*).type};
...
//
/+ template <class T>
size_t qHash(const QSharedPointer<T> &ptr, size_t seed = 0)
{
...
extern(C++, "QtSharedPointer") {
// helper functions:
    pragma(inline, true) QSharedPointer!(X) copyAndSetPointer(X, T)(X* ptr, ref const(QSharedPointer!(T)) src)
    {
...
// cast operators
pragma(inline, true) QSharedPointer!(X) qSharedPointerCast(X, T)(ref const(QSharedPointer!(T)) src)
{
    X* ptr = static_cast!(X*)(src.data()); // if you get an error in this line, the cast is invalid
    return /+ QtSharedPointer:: +/copyAndSetPointer(ptr, src);
}
pragma(inline, true) QSharedPointer!(X) qSharedPointerCast(X, T)(ref const(QWeakPointer!(T)) src)
{
    return qSharedPointerCast!(X, T)(src.toStrongRef());
}

pragma(inline, true) QSharedPointer!(X) qSharedPointerDynamicCast(X, T)(ref const(QSharedPointer!(T)) src)
{
...
pragma(inline, true) QSharedPointer!(X) qSharedPointerDynamicCast(X, T)(ref const(QWeakPointer!(T)) src)
{
    return qSharedPointerDynamicCast!(X, T)(src.toStrongRef());
}

pragma(inline, true) QSharedPointer!(X) qSharedPointerConstCast(X, T)(ref const(QSharedPointer!(T)) src)
{
    X* ptr = const_cast!(X*)(src.data()); // if you get an error in this line, the cast is invalid
    return /+ QtSharedPointer:: +/copyAndSetPointer(ptr, src);
}
pragma(inline, true) QSharedPointer!(X) qSharedPointerConstCast(X, T)(ref const(QWeakPointer!(T)) src)
{
    return qSharedPointerConstCast!(X, T)(src.toStrongRef());
}

pragma(inline, true) QWeakPointer!(X) qWeakPointerCast(X, T)(ref const(QSharedPointer!(T)) src)
{
