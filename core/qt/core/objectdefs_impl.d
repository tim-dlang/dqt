 ...
 */
module qt.core.objectdefs_impl;
extern(C++):
...
extern(C++, "QtPrivate") {
    ...
    */
    ...
    /+void operator ,(T)(T, ref const(ApplyReturnValue!(void)) ) {}+/
...
/+    pragma(inline, true) void assertObjectType(ObjPrivate)(QObjectPrivate* d);
    ...
    struct NarrowingDetector(T) { T[1] t; } // from P0608

    struct IsConvertibleWithoutNarrowing(From, To, Enable) {
    ...
    struct AreArgumentsConvertibleWithoutNarrowingBase(From, To, Enable) {
        /+ std:: +/false_type base0;
        alias base0 this;
}
    ...
    */
    ...
    extern(C++, class) struct QSlotObjectBase {
    private:
        QAtomicInt m_ref;
    ...
    public:
        /+ explicit +/this(ImplFn fn)
        {
            this.m_ref = QAtomicInt(1);
            this.m_impl = fn;
        }
...
@disable this(this);
/+@disable this(ref const(QSlotObjectBase));+/@disable ref QSlotObjectBase opAssign(ref const(QSlotObjectBase));    }
...
extern(D) struct DQtStaticSlotObject(Dg, Params...) if (is(Dg == delegate))
{
    ...
    extern(C++) static void impl(int which, QSlotObjectBase *this_, QObject r, void **a, bool *ret)
    {
        ...
        switch (which) {
        case QSlotObjectBase.Operation.Destroy:
...
extern(D) struct DQtMemberSlotObject(T, alias F, Params...) if (is(T: QObject))
{
    ...
    /* Need to use a custom mangling, because the template parameters
     * may not have a C++ mangling. */
    pragma(mangle, DQtMemberSlotObject.mangleof ~ "__impl")
    extern(C++) static void impl(int which, QSlotObjectBase *this_, QObject r, void **a, bool *ret)
    {
        import core.stdcpp.new_;
        import std.traits;
        switch (which) {
        case QSlotObjectBase.Operation.Destroy:
            cpp_delete(cast(DQtMemberSlotObject*)(this_));
            break;
        case QSlotObjectBase.Operation.Call:
            mixin("extern(" ~ functionLinkage!F ~ ") ReturnType!F delegate(Parameters!F) dg;");
            dg.ptr = cast(void*)r;
            dg.funcptr = &F;
            mixin((){
                import std.conv;
