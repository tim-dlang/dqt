...
extern(C++, "QtPrivate")
{
    ...
    /+struct is_complete(T, ODR_VIOLATION_PREVENTER) {
        /+ detail:: +/is_complete_helper!(/+ std:: +/remove_reference_t!(T), ODR_VIOLATION_PREVENTER).is_complete_helper.type base0;
        alias base0 this;
}+/

    struct MetatypeDecay(T)              { alias type = Unqual!T; }
...
struct QMetaTypeIdQObject(T)
{
    static if (IsPointerToTypeDerivedFromQObject!T.Value)
    {
        ...
        static int qt_metatype_id()
        {
            ...
            import qt.core.basicatomic;
            /+ Q_CONSTINIT +/ static QBasicAtomicInt metatype_id = QBasicAtomicInt(0);
...
extern(D) private template getCppNamespacesPrefix(alias T)
{
    static if (is(T : pair!(A, B), A, B))
        enum getCppNamespacesPrefix = "std::";
    else static if (!isCppSymbol!T)
...
{
    static immutable(char[fullyQualifiedNameCpp!T.length + 1]) result = fullyQualifiedNameCpp!(T, QualifiedNameCppFlags.ignoreHeadConst);
...
/+ pragma(inline, true) size_t qHash(QMetaType type, size_t seed = 0)
{
