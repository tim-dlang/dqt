...
extern(D) alias QVERIFY = function string(string statement)
{
    return
    mixin(interpolateMixin(q{    do {
            if (!/+ QTest:: +/qt.test.testcase.qVerify(static_cast!(bool)($(statement)), $(stringifyMacroParameter(statement)), "", __FILE__, __LINE__))
                return;
        } while (false);}));
};
...
extern(D) alias QFAIL = function string(string message)
{
    return
    mixin(interpolateMixin(q{    do {
            /+ QTest:: +/qt.test.testcase.qFail(static_cast!(const(char)*)($(message)), __FILE__, __LINE__);
            return;
        } while (false);}));
};
...
extern(D) alias QCOMPARE = function string(string actual, string expected)
{
    return
    mixin(interpolateMixin(q{    do {
            if (!/+ QTest:: +/qCompare($(actual), $(expected), $(stringifyMacroParameter(actual)), $(stringifyMacroParameter(expected)), __FILE__, __LINE__))
                return;
        } while (false);}));
};
...
pragma(inline, true) void useVerifyThrowsException() {}
} // namespace QTest
...
extern(C++, "QTest")
{
    ...
    /+ Q_TESTLIB_EXPORT +/ void qInit(QObject testObject__1, int argc = 0, char** argv = null);
    /+ Q_TESTLIB_EXPORT +/ int qRun();
    /+ Q_TESTLIB_EXPORT +/ void qCleanup();

    /+ Q_TESTLIB_EXPORT +/ int qExec(QObject testObject__1, int argc = 0, char** argv = null);
    /+ Q_TESTLIB_EXPORT +/ int qExec(QObject testObject__1, ref const(QStringList) arguments);

    /+ Q_TESTLIB_EXPORT +/ void setMainSourcePath(const(char)* file, const(char)* builddir = null);
...
/+ #if QT_DEPRECATED_SINCE(6, 3) +/
    /+ QT_DEPRECATED_VERSION_X_6_3("Use qWarning() instead") +/
        /+ Q_TESTLIB_EXPORT +/ void qWarn(const(char)* message, const(char)* file = null, int line = 0);
/+ #endif +/
    /+ Q_TESTLIB_EXPORT +/ void ignoreMessage(QtMsgType type, const(char)* message);
/+ #if QT_CONFIG(regularexpression) +/
    /+ Q_TESTLIB_EXPORT +/ void ignoreMessage(QtMsgType type, ref const(QRegularExpression) messagePattern);
/+ #endif +/
    /+ Q_TESTLIB_EXPORT +/ void failOnWarning(const(char)* message);
/+ #if QT_CONFIG(regularexpression) +/
    /+ Q_TESTLIB_EXPORT +/ void failOnWarning(ref const(QRegularExpression) messagePattern);
/+ #endif
    ...
    /+ Q_TESTLIB_EXPORT +/ /+ Qt:: +/qt.core.namespace.Key asciiToKey(char ascii);
    /+ Q_TESTLIB_EXPORT +/ char keyToAscii(/+ Qt:: +/qt.core.namespace.Key key);
...
/+ #if QT_DEPRECATED_SINCE(6, 4)
    ...
    pragma(inline, true) void addColumn(T)(const(char)* name, T*  /+ = nullptr +/)
    {
        import qt.core.metatype;

        alias QIsSameTConstChar = /+ std:: +/is_same!(T, const(char)*);
        static assert(!QIsSameTConstChar.value, "const char* is not allowed as a test data format.");
...
/+ #ifndef Q_QDOC
    ...
    pragma(inline, true) bool qTest(T)(ref const(T) actual, const(char)* elementName, const(char)* actualStr,
                         const(char)* expected, const(char)* file, int line)
    {
        import qt.core.metatype;

        return qCompare(actual, *static_cast!(const(T)*)(qElementData(elementName,
